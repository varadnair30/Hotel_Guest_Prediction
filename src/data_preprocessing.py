...
class DataPreprocessing:
    ...
    def preprocess_data(self,df):
        try:
            ...
            for col in cat_cols:
                df[col]=label_enocoder.fit_transform(df[col])
                mappings[col]={label:code for label,code in zip(label_enocoder.classes_,label_enocoder.transform(label_enocoder.classes_))}
            ...
            logger.error(f"Error occured in our data preprocessing step {e}")
    ...
    def balance_data(self,df):
        try:
            ...
            logger.error(f"Error during data balancing data step {e}")
    ...
    def select_features(self,df):
        try:
            ...
            top_feature_importance_df=feature_importance_df.sort_values('importance',ascending=False)
            
            num_features_to_select=self.config["data_preprocessing"]["no_of_top_features"]
            
            top_10_features = top_feature_importance_df.head(num_features_to_select)["feature"].tolist()
    ...
    def save_data(self,df,file_path):
        try:
            logger.info("Saving pur data in processed folder")
