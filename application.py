...
@app.route('/',methods=['GET','POST'])  #Get for getting the index.html without prediction and POST for posting the predictions on frontend when the user clicks on predict button
def index():
    
    if request.method=='POST': #If there is post request from HTML. We will get data from HTML to flask
